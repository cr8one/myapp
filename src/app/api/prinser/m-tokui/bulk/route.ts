...
function parseShiftJisCsv(buffer: Buffer): Record<string, string>[] {
  ...
  return lines.map((line: string) => {
    ...
    r.tokuisaki_cd?.trim() && r.siten_cd?.trim()
  )
}
...
export async function POST(req: NextRequest) {
  ...
  const allRecords = parseShiftJisCsv(Buffer.concat(chunks))

  const slice = allRecords.slice(offset, offset + CHUNK_SIZE)
  if (slice.length === 0) return NextResponse.json({ ok: true, count: 0, done: true })

  await prisma.prinserMTokui.createMany({
    data: slice.map(r => ({
      tokuisaki_cd: r.tokuisaki_cd, siten_cd: r.siten_cd,
      ...
      tantosya_cd: r.tantosya_cd || null, tiiki_cd: r.tiiki_cd || null,
      gyosyu_cd: r.gyosyu_cd || null, best_juni: r.best_juni || null,
      kyakusaki_bunrui: r.kyakusaki_bunrui || null, gaityu_bunrui: r.gaityu_bunrui || null,
      nokyo_kbn: r.nokyo_kbn || null, sime_date: r.sime_date || null,
      zei_kbn: r.zei_kbn || null, aitesaki_kbn: r.aitesaki_kbn || null,
      hon_kari_kbn: r.hon_kari_kbn || null, president: r.president || null,
      kabu: r.kabu || null, oya_tokui_cd: r.oya_tokui_cd || null,
      ...
      tmail: r.tmail || null, kagami_flg: r.kagami_flg || null,
      ky_mon: r.ky_mon || null, ky_dt: r.ky_dt || null,
      zzan_sei_flg: r.zzan_sei_flg || null, ny_houhou: r.ny_houhou || null,
  ...
  const done = offset + CHUNK_SIZE >= allRecords.length
  return NextResponse.json({ ok: true, count: slice.length, total: allRecords.length, done })
}
