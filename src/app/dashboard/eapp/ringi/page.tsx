...
const PAGE_SIZE = 50
const STATUS_OPTIONS = ["下書き", "起案部承認中", "受付待ち", "関連部承認中", "決裁済み"] as const
const STATUS_STYLE: Record<string, string> = {
  "下書き": "bg-gray-100 text-gray-600",
  "起案部承認中": "bg-amber-100 text-amber-700",
  "受付待ち": "bg-orange-100 text-orange-700",
  "関連部承認中": "bg-blue-100 text-blue-700",
  "決裁済み": "bg-emerald-100 text-emerald-700",
}

export default function EAppRingiPage() {
  const router = useRouter()
  const [records, setRecords] = useState<RingiRequest[]>([])
  ...
  const fetchRecords = async (p = page, kw = keyword, st = statusFilter) => {
    ...
    const res = await fetch(`/api/eapp/ringi?${params.toString()}`)
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">稟議書</h1>
        <Button onClick={() => router.push("/dashboard/eapp/ringi/new")}>新規作成</Button>
      </div>
            ...
            <table className="w-full table-fixed">
              ...
              <thead>
                <tr className="border-b bg-gray-50 text-xs text-gray-500">
                  <th className="text-left font-medium px-3 py-3">件名</th>
                  <th className="text-left font-medium px-3 py-3">起案者</th>
                  <th className="text-left font-medium px-3 py-3">ステータス</th>
                  <th className="text-left font-medium px-3 py-3">受付番号</th>
                  <th className="text-left font-medium px-3 py-3">決裁日</th>
              ...
              <tbody>
                {records.map(r => (
                  <tr
                    ...
                    onClick={() => router.push(`/dashboard/eapp/ringi/${r.id}`)}
                  >
                    <td className="px-3 py-4">
                      <span className="text-sm text-gray-700 break-words leading-snug">{r.title}</span>
                    </td>
                    <td className="px-3 py-4">
                      <span className="text-sm text-gray-600 break-words leading-snug">{r.requester_names}</span>
                    </td>
                    <td className="px-3 py-4">
                    ...
                    <td className="px-3 py-4 text-sm text-gray-600">{r.reception_number ?? ""}</td>
                    <td className="px-3 py-4 text-sm text-gray-600">{formatDate(r.decision_date)}</td>
                    <td className="px-3 py-4" onClick={e => e.stopPropagation()}>
                      <Button variant="outline" size="sm"
                        onClick={() => router.push(`/dashboard/eapp/ringi/${r.id}`)}>詳細</Button>
                    </td>
                  </tr>
                ))}
              </tbody>
