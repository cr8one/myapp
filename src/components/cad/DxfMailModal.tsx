...
import { dxfRequestMailCategory, dxfCompleteMailCategory } from "@/lib/cad/mail-categories"
...
export default function DxfMailModal({ record, mode, onClose, onSent }: Props) {
  ...
  const category = mode === "request" ? dxfRequestMailCategory(record.purpose) : dxfCompleteMailCategory(record.purpose)
  const title = mode === "request" ? "DXF変換依頼メール送信" : "DXF変換完了メール送信"
  const subjectPrefix = mode === "request" ? "DXF変換依頼" : "DXF変換完了"
  const endpoint = mode === "request" ? "request-mail" : "complete-mail"

  useEffect(() => {
    Promise.all([
      fetch(`/api/cad/masters/mail-recipients?category=${category}`).then(r => r.json()),
      fetch(`/api/cad/masters/mail-template?category=${category}`).then(r => r.json()),
      fetch("/api/users/list").then(r => r.json()),
    ]).then(([recipients, template, userList]) => {
      setToRecipients(recipients)
      setBody(template.body ?? "")
      setUsers(userList)
      setSubject(`${subjectPrefix} No${record.uid}_${record.purpose ?? ""}`)
      setLoading(false)
    })
  }, [record, category, subjectPrefix])
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch(`/api/cad/dxf-requests/${record.id}/${endpoint}`, {
        method: "POST",
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">DXF変換依頼書 No.{record.uid}</p>
          </div>
